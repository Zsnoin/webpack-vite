...
var Literal = class extends NodeBase {
  ...
  getLiteralValueAtPath(path2) {
    if (path2.length > 0 || this.value === null && this.context.code.charCodeAt(this.start) !== 110 || typeof this.value === "bigint" || this.context.code.charCodeAt(this.start) === 47) {
      return UnknownValue;
    }
    return this.value;
  }
...
var MemberExpression = class extends NodeBase {
  ...
  applyDeoptimizations() {
    this.deoptimized = true;
    const { propertyReadSideEffects } = this.context.options.treeshake;
    if (this.bound && propertyReadSideEffects && !(this.variable || this.isUndefined)) {
      const propertyKey = this.getPropertyKey();
...
var ClassNode = class extends NodeBase {
  ...
  hasEffectsOnInteractionAtPath(path2, interaction, context) {
    var _a;
    return interaction.type === INTERACTION_CALLED && path2.length === 0 ? !interaction.withNew || (this.classConstructor !== null ? this.classConstructor.hasEffectsOnInteractionAtPath(path2, interaction, context) : (_a = this.superClass) == null ? void 0 : _a.hasEffectsOnInteractionAtPath(path2, interaction, context)) || false : this.getObjectEntity().hasEffectsOnInteractionAtPath(path2, interaction, context);
  }
  include(context, includeChildrenRecursively) {
    var _a;
    if (!this.deoptimized)
...
function getAndExtendSideEffectModules(variable, module) {
  const sideEffectModules = getOrCreate(module.sideEffectDependenciesByVariable, variable, () => /* @__PURE__ */ new Set());
...
function warnOnBuiltins(warn, dependencies) {
  const externalBuiltins = dependencies.map(({ importPath }) => importPath).filter((importPath) => importPath in builtins || importPath.startsWith("node:"));
...
var Chunk2 = class {
  ...
  getFileName() {
    var _a;
    return ((_a = this.preliminaryFileName) == null ? void 0 : _a.fileName) || this.getPreliminaryFileName().fileName;
  }
  getImportPath(importer) {
...
function getChunkAssignments(entryModules, manualChunkAliasByEntry, minChunkSize) {
  ...
  function assignEntryToStaticDependencies(entry, dynamicDependentEntryPoints) {
    ...
    for (const module of modulesToHandle) {
      const assignedEntryPoints = getOrCreate(assignedEntryPointsByModule, module, () => /* @__PURE__ */ new Set());
...
function analyzeModuleGraph(entryModules) {
  ...
  for (const currentEntry of entriesToHandle) {
    ...
    for (const module of modulesToHandle) {
      getOrCreate(dependentEntryPointsByModule, module, () => /* @__PURE__ */ new Set()).add(currentEntry);
      for (const dependency of module.getDependenciesToBeIncluded()) {
        if (!(dependency instanceof ExternalModule)) {
          modulesToHandle.add(dependency);
        }
...
async function findFile(file, preserveSymlinks) {
  try {
    const stats = await import_node_fs.promises.lstat(file);
    if (!preserveSymlinks && stats.isSymbolicLink())
      return await findFile(await import_node_fs.promises.realpath(file), preserveSymlinks);
    if (preserveSymlinks && stats.isSymbolicLink() || stats.isFile()) {
      const name = (0, import_node_path.basename)(file);
      const files = await import_node_fs.promises.readdir((0, import_node_path.dirname)(file));
...
async function transform(source, module, pluginDriver, warn) {
  ...
  if (!customTransformCache && emittedFiles.length > 0)
...
var ModuleLoader = class {
  ...
  async addModuleSource(id, importer, module) {
    let source;
    try {
      source = await this.graph.fileOperationQueue.run(async () => await this.pluginDriver.hookFirst("load", [id]) ?? await import_node_fs.promises.readFile(id, "utf8"));
    } catch (error_) {
...
var FileEmitter = class {
  constructor(graph, options, baseFileEmitter) {
    ...
    this.setOutputBundle = (bundle, outputOptions) => {
      const fileNamesBySource = /* @__PURE__ */ new Map();
  ...
  assignReferenceId(file, idBase) {
    let referenceId;
    do {
      referenceId = createHash().update(referenceId || idBase).digest("hex").slice(0, 8);
...
async function writeOutputFile(outputFile, outputOptions) {
  ...
  await import_node_fs.promises.mkdir((0, import_node_path.dirname)(fileName), { recursive: true });
  return import_node_fs.promises.writeFile(fileName, outputFile.type === "asset" ? outputFile.source : outputFile.code);
}
...
function handleError(error2, recover = false) {
  ...
  if (error2.url) {
    stderr(cyan(error2.url));
  }
  if (error2.loc) {
    stderr(`${relativeId(error2.loc.file || error2.id)} (${error2.loc.line}:${error2.loc.column})`);
  } else if (error2.id) {
    stderr(relativeId(error2.id));
  }
  if (error2.frame) {
    stderr(dim(error2.frame));
  }
  if (error2.stack) {
    stderr(dim(error2.stack));
  }
  stderr("");
  if (!recover)
...
async function watchInternal(configs, emitter) {
  ...
  await loadFsEvents();
  const { Watcher } = await import("./watch-RB7MARWF.js");
...
//# sourceMappingURL=chunk-CW6LKNOY.js.map
